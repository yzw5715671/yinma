    ...
    <div class="userright2 ri">
        <table>
            ...
            <?php if(is_array($projects)): $i = 0; $__LIST__ = $projects;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td class="img"><img src="<?php echo (get_cover($vo["cover"],'path')); ?>"></td>
                    <td><a href="<?php echo U('Project/detail?id='.$vo['id']);?>"><?php echo ($vo["project_name"]); ?></a>
                        <em class="date">创建时间：<?php echo (time_format($vo["create_time"],'Y-m-d H:s')); ?></em></td>
                    <td>
                        <?php if($vo["stage"] == 0): ?>未上线
                            <?php elseif($vo["stage"] == 1): ?>
                            预热中
                            <?php elseif($vo["stage"] == 2): ?>
                            询价中
                            <?php elseif($vo["stage"] == 3): ?>
                            推举领投
                    ...
                    <td class="cz">
                        <?php if(($vo["status"] == 9) AND ($vo["stage"] > 1)): ?><a href="<?php echo U('Manage/statistical?id='.$vo['id']);?>">认投统计</a><?php endif; ?>
                        <?php if(($vo["status"] == 9) AND ($vo["stage"] >= 1) AND ($vo["vote_leader"] != 2)): ?><a href="<?php echo U('MCenter/pj_leaders?id='.$vo['id']);?>">候选领投</a><?php endif; ?>
                        <?php if($vo["stage"] < 3): ?><a href="<?php echo U('Project/addstep1?id='.$vo['id']);?>">编辑</a><?php endif; ?>
                        <?php if($vo["status"] == 0): ?><a href="<?php echo U('Project/addstep6?id='.$vo['id']);?>">提交审核</a><?php endif; ?>
                        <?php if(($vo["status"] == 9) AND ($vo["stage"] == 9)): ?><a class="manage" href="<?php echo U('projectAfterInfo/fundedmanagelist?pid='.$vo['id']);?>" target="_blank">投后管理</a><?php endif; ?>
                    </td>
