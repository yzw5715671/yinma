...
<ul>
  <li class="cur"><a href="<?php echo U('MCenter/pr_support');?>">支持的项目</a></li>
  <li><a href="<?php echo U('MCenter/pr_create');?>">发起的项目</a></li>
...
<table>
  <thead>
	...
	<tbody>
    <?php if(is_array($supportList)): $i = 0; $__LIST__ = $supportList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
	    <td width="120" class="img">
			<a href="<?php echo U('Product/viewdetail?pid='.$vo['id']);?>"><img src="<?php echo (get_cover($vo["home_img"],'path')); ?>"></a>
		</td>
	    <td width="200">
	    	<a href="<?php echo U('Product/viewdetail?pid='.$vo['id']);?>"><?php echo ($vo["name"]); ?></a>
	    	<em class="z"><?php echo ($vo["stage_text"]); ?></em>
	    	<em class="z"><?php echo ($vo["shareno"]); ?></em>
	    </td>
	    <td><?php echo (time_format($vo["create_time"],"Y-m-d")); ?></td>
	    <td style="text-align:center;">¥<?php echo ($vo["amount"]); ?><br>（运费：¥<?php echo ($vo["post_amount"]); ?>）</td>
	    <td><b class="f14"><?php echo ($vo["luckno"]); ?></b></td>
	    <td><!-- 状态 回报相关id（0:购买成功、1:支付成功、-1:撤消 -2没有购买名额了） -->
			<?php if($vo['status'] == 0): ?>购买成功
				<?php elseif($vo['status'] == 1): ?>支付成功
				<?php elseif($vo['status'] == -1): ?>已撤销
				<?php elseif($vo['status'] == -2): ?>无购买名额<?php endif; ?>
		</td>
	    <td class="cz">
	 		<?php if($vo['status'] == 0): ?><a class="btn btn-warning btn-small" href="<?php echo U('ProductOrder/confirm?customId='.$vo['customid']);?>" >支付</a>
				<a class="IxD-revocation" href="#" data-url="<?php echo U('ProductOrder/revocation?customId='.$vo['customid']);?>" >撤销</a>
