...
<ul>
    <li><a href="<?php echo U('MCenter/pr_support');?>">支持的项目</a></li>
    <li class="cur"><a href="<?php echo U('MCenter/pr_create');?>">发起的项目</a></li>
   <!--  <li><a href="<?php echo U('MCenter/pr_attach');?>">收藏的项目</a></li> -->
</ul>
...
<tbody>
    <?php if(is_array($myProductList)): $i = 0; $__LIST__ = $myProductList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
	    <td width="120" class="img">
			<a href="<?php echo U('Product/viewdetail?pid='.$vo['id']);?>"><img src="<?php echo (get_cover($vo["home_img"],'path')); ?>"></a>
		</td>
	    <td width="250">
	    	<a href="<?php echo U('Product/viewdetail?pid='.$vo['id']);?>" target="_blank"><?php echo ($vo["name"]); ?></a>
	    	<em class="date">发起时间：<?php echo (time_format($vo["create_time"],"Y-m-d")); ?></em></td>
	    <td style="text-align:center;">
	    	¥<?php echo ($vo['amount']); ?><br>（当前进度<?php echo round($vo['finish_amount']/$vo['amount'],2) * 100;?>%）
	    </td>
	    <td>
			...
			<?php elseif($vo['status'] == 9): ?>
				...
				<?php elseif($vo['stage'] == 9): ?>众筹成功<?php endif; endif; ?>
		</td>
	    <td class="cz">
			<?php if(in_array(($vo["status"]), explode(',',"0,2"))): ?><a href="<?php echo U('Product/addstep4?pid='.$vo['id']);?>">提交审核</a><?php endif; ?>
	    	<?php if(in_array(($vo["status"]), explode(',',"0,1,2"))): ?><a class="btn btn-small" href="<?php echo U('Product/addstep1?pid='.$vo[id].'&reviseStatus=true');?>" target="_blank">修改</a>
				<!-- <a href="#">删除</a> --><?php endif; ?>
			<?php if(in_array(($vo["stage"]), explode(',',"2,8,9"))): ?><a class="btn btn-small" href="<?php echo U('Manage/myCrowdfundingOrder?pid='.$vo[id]);?>" target="_blank">查看订单</a><?php endif; ?>
	    	<a class="btn btn-small" href="<?php echo U('Product/viewdetail?pid='.$vo['id']);?>" target="_blank">预览</a>
	    </td>
...
<script type="text/javascript">
  // 指定左侧选中菜单
  var submenu = "<?php echo U('MCenter/pr_support');?>";
</script>
 <!-- 用于加载js代码 -->
