...
<div class="fl w100 usermain mt15">
  ...
  <ul>
    ...
    <li class=><a href="<?php echo U('Account/cardlist');?>">银行卡管理</a></li>
  </ul>
...
<div class="userright2 ri">
  <div class="border1 zc clearfix" style="text-align:left">
    <div style="float:left;margin-left:30px; margin-top:5px;">
      账户余额：<em><?php echo ((isset($data["balance"]) && ($data["balance"] !== ""))?($data["balance"]):0); ?></em><i>元</i>
      可提现金额：<em><?php echo ((isset($data["use_able"]) && ($data["use_able"] !== ""))?($data["use_able"]):0); ?></em><i>元</i>
    <!--  冻结资金：<em>49500</em><i>元</i> -->
    </div>
    <div style="float:right;margin-right:30px">
      <a href="<?php echo U('Account/recharge');?>" id="recharge" data-w="500px" data-h="300px" class="poplight">充值</a>
      <a href="<?php echo U('Account/cash');?>" data-w="500px" data-h="440px" class="poplight">提现</a>
    </div>
  </div>
  <div class="czrecords mt20">
    <div class="top">
      <header><b>操作记录</b></header>
      <?php if(empty($list)): ?><div class="norecord">暂无交易记录</div>
      <?php else: ?>
    ...
    <table>
      ...
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
        <td><?php echo ($i); ?></td>
        <td><?php if($vo["type"] == 0): ?>充值
        <?php elseif($vo["type"] == 1): ?>支付
        <?php elseif($vo["type"] == 2): ?>赎回
        <?php elseif($vo["type"] == 3): ?>提现<?php endif; ?></td>
        <td><?php echo (time_format($vo["create_time"],"Y-m-d H:i:s")); ?></td>
        <td>¥<?php echo ($vo["amount"]); ?></td>
        <td><?php if($vo["state"] == 0): ?>未处理
        <?php elseif($vo["state"] == 1): ?>完成
        <?php elseif($vo["state"] == 2): ?><a href="#" class="error_info" title="点击查看原因" data-info="<?php echo ($vo["remarks"]); ?>">失败</a><?php endif; ?></td>
