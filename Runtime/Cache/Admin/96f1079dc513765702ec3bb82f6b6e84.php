		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($channel["extra"]); ?></td>
						<td><?php echo ($channel["remark"]); ?></td>
                        <td><?php echo ($channel["sort"]); ?></td>
                        <td>
	                        <?php if($channel['status'] == 0): ?><span class="label">可用</span>
								<?php elseif($channel['status'] == 1): ?><span class="label label-success">禁用</span><?php endif; ?>
                        </td>
						<td>
...
<!-- 							<a href="<?php echo U('setStatus?ids='.$channel['id'].'&status='.abs(1-$channel['status']));?>" class="ajax-get"><?php echo (show_status_op($channel["status"])); ?></a> -->
							<?php if($channel['status'] == 0): ?><a class="confirm ajax-get"  href="<?php echo U('setStatus?id='.$channel['id'].'&status=1');?>">禁用</a>
							<?php elseif($channel['status'] == 1): ?>
								<a class="confirm ajax-get"  href="<?php echo U('setStatus?id='.$channel['id'].'&status=0');?>">启用</a><?php endif; ?>
