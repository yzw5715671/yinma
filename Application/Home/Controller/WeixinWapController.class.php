 ...
 * 包括用户中心，用户登录及注册
 */
class WeixinWapController extends HomeController {
	function getConfig(){
		$config = array(
				'APPID' => 'wxe9563c6cf01c4858',
				'SECRET' => 'b38eb0d3c2b44fe68d5988db5bb0cfbd',
				'STATE' => '1tht2016',
	...
	function getWeixinData(){
		...
		$data['redirect_uri'] = urlencode('http://'. $_SERVER['HTTP_HOST'].U('WeixinWap/bindWx'));
	...
	function loginWxLink(){
		if(isMobile() ){// & strpos($_SERVER["HTTP_USER_AGENT"],"MicroMessenger")
			...
			$config = $this->getConfig();
			$state = $config['STATE'];
			$redirect_uri= urlencode('http://'. $_SERVER['HTTP_HOST'].U('WeixinWap/loginWx'));
			$wx_url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$config['APPID'].'&redirect_uri='.$redirect_uri.'&response_type=code&scope=snsapi_userinfo&state='.$config['STATE'].'#wechat_redirect';
	...
	function bindWxLink(){
		if(isMobile() ){// & strpos($_SERVER["HTTP_USER_AGENT"],"MicroMessenger")
			$config = $this->getConfig();
			$state = $config['STATE'];
			$redirect_uri= urlencode('http://'. $_SERVER['HTTP_HOST'].U('WeixinWap/bindWx'));
	...
	function loginWx(){
		...
		if($state ===  $config['STATE']){
			...
			$openid = $info['openid'];
			$unionid = $info['unionid'].rand(1000,9999);
					...
					if($checkuser){
						if(D('Users')->login($checkuser['id'])){ //登录用户
							if($pid>0){
								...
								if (empty($url)) {$url = U('MCenter/index');}
								$this->success('登录成功！',$url);
							}
                        }
					}else{
							// $this->redirect('User/login', array('type' => 'bind','userinfo'=>base64_encode(json_encode($user_info))));
							$this->redirect('User/login', array('type' => 'bind','openid'=>$openid,'token'=>$unionid));
		...
		if ($_GET['password'] & $_GET['username']) {
			...
			if(0 < $uid){ //UC登录成功
				...
				if($Member->login($uid)){ //登录用户
						...
						$currentUser['unionid'] = substr($_GET['token'],0,-4);
						$update = M('UcenterMember')->save($currentUser);
						if($update==1){
	...
	public function weixinlogin(){
		...
		redirect('https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$config['APPID'].'&redirect_uri=http://www.1tht.cn/WeixinWap/loginWx/&response_type=code&scope=snsapi_userinfo&state='.$config['STATE'].'#wechat_redirect');
	}
}
?>